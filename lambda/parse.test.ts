...
describe('parseLambdaMessage', () => {
  ...
  test('parsing a END message should yield no msg field', () => {
    const parsed = parseLambdaMessage('END RequestId: bf155fd9-a3d2-4001-be3b-e69afc5816c8')
    expect(parsed.msg).toEqual('')
  })
    ...
    const invokeError = `2019-11-04T08:36:24.848Z 7f0f1a90-5dc4-409a-b737-e75bb81a72a8 ERROR Invoke Error
  {
    ...
    const parsed = parseLambdaMessage(invokeError)
    expect(parsed.reqid).toBe('7f0f1a90-5dc4-409a-b737-e75bb81a72a8')
    expect(parsed.level).toBe('error')
    expect(parsed.msg).toBe('some error message')
...
describe('parseAppMessage', () => {
  it('should extract unstructured text', () => {
    const log = parseAppMessage('unstructured text')
